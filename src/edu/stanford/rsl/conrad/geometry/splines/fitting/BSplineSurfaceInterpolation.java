...
public class BSplineSurfaceInterpolation {
	...
	public SurfaceBSpline applyInterpolation(int parametrization){
		uParameters = buildUParameterVector();
		vParameters = buildVParameterVector();
		double[] uKnots = buildKnotVector(uParameters, degreeU);
		System.out.println("uKnot:" + Arrays.toString(uKnots));
		double[] vKnots = buildKnotVector(vParameters, degreeV);
		...
		for(int l = 0; l < m; l ++){
			...
			ArrayList<PointND> temp = uSplineInterpolation.applyInterpolation(uParameters, uKnots, BSplineCurveInterpolation.CLAMPED).getControlPoints();
		...
		//	System.out.println("VCP_inter"+vPoints);
			...
			ArrayList<PointND> temp = vSplineInterpolation.applyInterpolation(vParameters, vKnots, BSplineCurveInterpolation.CLAMPED).getControlPoints();
	...
	/**
	 * Computes the u-parameter vector. For reference see NURBS book Chapter 9.2.5
	 * @return the parameter vector
	 */
	protected double[] buildUParameterVector(){
		double[] uParameters = new double[n];
		uParameters[n - 1] = 1d;
		int num = m;
		//compute cds for each column
		for(int l = 0; l < m; l++){
			double total = 0d;
			double[] cds = new double[n];
			//compute for u-vector for current column l
			for (int k = 1; k < n; k ++){
		...
		if(num == 0) throw new RuntimeException(this.getClass().getName() + "Error building u-parameter vector.");
		for(int k = 1; k < n - 1; k ++) uParameters[k] /= num;		
		return uParameters;
	}
	
	/**
	 * Computes the v-parameter vector. For reference see NURBS book Chapter 9.2.5
	 * @return the parameter vector
	 */
	protected double[] buildVParameterVector(){
		...
		if(num == 0) throw new RuntimeException(this.getClass().getName() + "Error building v-parameter vector.");
	...
	public static void main(String[] args) {
		...
		SurfaceBSpline Sbsp = test.applyInterpolation(CHORD);
